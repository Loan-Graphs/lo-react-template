...
import {
  ...
  calculateUsdaLoanAmount,
  calculateUsdaMonthlyInsurance,
...
export default function USDACalculator() {
  ...
  const [downPayment, setDownPayment] = useState(0); // USDA: 0% down
  ...
  const [guaranteeFeeRate] = useState(1); // 1% USDA upfront guarantee fee
  const [annualFeeRate] = useState(0.35); // 0.35% annual fee

  const dpAmount = useMemo(() => calculateDownPayment(homePrice, downPayment, "%"), [homePrice, downPayment]);
  const usdaDetails = useMemo(
    () => calculateUsdaLoanAmount(homePrice, dpAmount, guaranteeFeeRate),
    [homePrice, dpAmount, guaranteeFeeRate]
  );
  const { baseLoanAmount, guaranteeFee, usdaLoanAmount } = usdaDetails;
  const usdaMonthlyIns = useMemo(
    () => calculateUsdaMonthlyInsurance(usdaLoanAmount, annualFeeRate),
    [usdaLoanAmount, annualFeeRate]
  );
  const ltv = useMemo(() => calculateLTV(baseLoanAmount, homePrice), [baseLoanAmount, homePrice]);

  const monthlyPI = useMemo(
    () => calculateMonthlyPayment(usdaLoanAmount, interestRate, loanTerm),
    [usdaLoanAmount, interestRate, loanTerm]
  );
  ...
  const totalMonthly = useMemo(
    () => calculateTotalMonthlyPayment(monthlyPI, monthlyTax, monthlyIns, hoaDues, usdaMonthlyIns),
    [monthlyPI, monthlyTax, monthlyIns, hoaDues, usdaMonthlyIns]
  );
  ...
  const pieSlices: PieSlice[] = [
    ...
    { value: usdaMonthlyIns, color: "#ef4444", label: "USDA Annual Fee" },
  ].filter((s) => s.value > 0);
      ...
      <div style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)" }} className="py-10">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span style={{ backgroundColor: "#15803d", color: "#fff", padding: "0.25rem 0.75rem", borderRadius: "999px", fontSize: "0.75rem", fontWeight: 700 }}>USDA</span>
            <h1 className="text-3xl font-bold" style={{ color: "#ffffff" }}>USDA Loan Affordability Calculator</h1>
          </div>
          <p style={{ color: "#94a3b8", maxWidth: "540px" }}>
            USDA Rural Development loans offer 100% financing (0% down) for eligible rural
            properties. Includes 1% upfront guarantee fee + 0.35% annual fee.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div style={{ backgroundColor: "#1e293b", borderRadius: "1rem", padding: "1.5rem", border: "1px solid #334155" }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input type="range" min={0} max={20000} step={100} value={grossIncome} onChange={(e) => setGrossIncome(Number(e.target.value))} style={{ width: "100%", accentColor: "#0ea5e9", marginTop: "0.4rem" }} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" min={50000} max={1000000} step={5000} value={homePrice} onChange={(e) => setHomePrice(Number(e.target.value))} style={inputCls} />
                <input type="range" min={50000} max={1000000} step={5000} value={homePrice} onChange={(e) => setHomePrice(Number(e.target.value))} style={{ width: "100%", accentColor: "#0ea5e9", marginTop: "0.4rem" }} />
              </div>
              <div>
                <label style={labelCls}>Down Payment % (0% for USDA)</label>
                <input type="number" min={0} max={20} step={0.5} value={downPayment} onChange={(e) => setDownPayment(Number(e.target.value))} style={inputCls} />
                <p style={{ color: "#64748b", fontSize: "0.75rem", marginTop: "0.25rem" }}>= {formatCurrency(dpAmount)} · LTV: {ltv.toFixed(1)}%</p>
              </div>
              <div>
                <label style={labelCls}>Loan Term</label>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  {[30].map((yr) => (
                    <button key={yr} onClick={() => setLoanTerm(yr)} style={{ flex: 1, padding: "0.5rem", borderRadius: "0.375rem", border: "2px solid #0ea5e9", backgroundColor: "rgba(14,165,233,0.15)", color: "#38bdf8", fontWeight: 700, cursor: "default", fontSize: "0.875rem" }}>
                      {yr} yr (USDA standard)
                    </button>
                  ))}
                </div>
              ...
              <div style={{ backgroundColor: "rgba(14,165,233,0.06)", border: "1px solid rgba(14,165,233,0.2)", borderRadius: "0.5rem", padding: "0.75rem", fontSize: "0.8rem" }}>
                <p style={{ color: "#38bdf8", fontWeight: 600, marginBottom: "0.4rem" }}>USDA Fees</p>
                <p style={{ color: "#94a3b8" }}>Upfront Guarantee Fee (1%): {formatCurrency(guaranteeFee)}</p>
                <p style={{ color: "#94a3b8" }}>USDA Loan Amount: {formatCurrency(usdaLoanAmount)}</p>
                <p style={{ color: "#94a3b8" }}>Annual Fee (0.35%): {formatCurrency(usdaMonthlyIns)}/mo</p>
                <p style={{ color: "#86efac", marginTop: "0.4rem" }}>✓ No PMI required · 0% down available</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div style={{ backgroundColor: "#1e293b", borderRadius: "1rem", padding: "1.5rem", border: "1px solid #334155", textAlign: "center" }}>
              <p style={{ color: "#94a3b8", fontSize: "0.85rem", marginBottom: "0.5rem" }}>Total Monthly USDA Payment</p>
            ...
            <div style={{ backgroundColor: "#1e293b", borderRadius: "1rem", padding: "1.5rem", border: "1px solid #334155" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem 1rem", fontSize: "0.85rem" }}>
                {[
                  ...
                  ["Base Loan", formatCurrency(baseLoanAmount)],
                  ["USDA Guarantee Fee", formatCurrency(guaranteeFee)],
                  ["USDA Loan Amount", formatCurrency(usdaLoanAmount)],
                  ["Down Payment", formatCurrency(dpAmount)],
                  ["LTV Ratio", `${ltv.toFixed(1)}%`],
                  ["Monthly P&I", formatCurrency(monthlyPI)],
                  ["Monthly USDA Fee", formatCurrency(usdaMonthlyIns)],
